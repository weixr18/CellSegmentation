...
class TestSet(Dataset):
    def __init__(self, txtpath, data_reader=load_file):
        ...
        with open(txtpath, 'r') as fh:
            for line in fh:
                ...
                """
                # 0,1,2分别是cell,GT,result路径
                data_paths.append((words[0], words[1], words[2]))
                """
...
def get_dataset(cell_dir, mask_dir, valid_rate, tmp_dir,
    ...
    use_exist_dataset = use_exist_dataset and os.path.isfile(
        valid_txt) and os.path.isfile(train_txt)

    if not use_exist_dataset:
...
def get_test_dataset(cell_dir, mask_dir, valid_rate, tmp_dir,
                     use_exist_dataset=True):
    ...
    # get the Dataset objects
    train_dataset = TestSet(train_txt, data_reader=load_file)
    valid_dataset = TestSet(valid_txt, data_reader=load_file)
    ...
    if not use_exist_dataset:
        ...
        cell_list = [os.path.join(cell_dir, image)
                     for image in os.listdir(cell_dir)]
        GT_list = [os.path.join(GT_dir, image)
                   for image in os.listdir(GT_dir)]
        res_list = [os.path.join(res_dir, image)
                    for image in os.listdir(res_dir)]

        # separate the lists according to valid_rate
        sample_size = len(cell_list)
        valid_index = np.random.choice(
            a=sample_size, size=sample_size, replace=False, p=None)

        # save the lists in txt files
        with open(valid_txt, "w+") as f:
            f.truncate()
            for i in valid_index:
                f.write(cell_list[i] + " " + GT_list[i] +
                        " " + res_list[i] + '\n')
